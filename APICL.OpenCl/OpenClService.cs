...
namespace APICL.OpenCl
{
	...
	{
		public string Repopath => Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "..", "..", "..", "..", "APICL.OpenCl"));
		...
		public async Task<IntPtr> ExecuteAudioKernel(AudioObj obj, string kernelBaseName = "normalize", string kernelVersion = "00", int chunkSize = 0, float overlap = 0.0f, Dictionary<string, object>? optionalArguments = null, bool log = false)
		{
			...
			obj.Pointer = this.KernelExecutioner.ExecuteAudioKernel((nint)obj.Pointer, out double factor, obj.Length, kernelBaseName + kernelVersion, chunkSize, overlap, obj.Samplerate, obj.Bitdepth, obj.Channels, optionalArguments, log);
		...
		public async Task<IntPtr> PerformFFT(AudioObj obj, int chunkSize = 0, float overlap = 0.0f, bool log = false)
		{
			...
			obj.Pointer = this.KernelExecutioner?.ExecuteFFT((nint)obj.Pointer, obj.Form.FirstOrDefault(), chunkSize, overlap, true, log) ?? obj.Pointer;
